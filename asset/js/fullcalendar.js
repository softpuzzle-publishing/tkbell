...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		titleHtml =
			'<cite onClick="fnCmdChangeDueDate(\''+ htmlEscape(event.endT || '') +'\')" class="'+(htmlEscape(event.subjectcode || '') || '&nbsp;')+' fc-title" data-subject="'+(htmlEscape(event.subject || '"') || '&nbsp;')+'">' +
				'<span>' + (htmlEscape(event.subject || '') || '&nbsp;') + '</span>'+
                '<span>' + (htmlEscape(event.grade || '') || '&nbsp;') + '</span>'+
            '</cite>' +
            '<p onClick="fnCmdChangeDueDate(\''+ htmlEscape(event.endT || '') +'\')" >'+
                '<strong>' + (htmlEscape(event.session || '') || '&nbsp;') + '</strong>' +
                '<span>' + (htmlEscape(event.title || '') || '&nbsp;') + '</span>'+// we always want one line of height
                '<span>' + (htmlEscape(event.page || '') || '&nbsp;') + '</span>' +
            '</p>' +
            '<time onClick="fnCmdChangeDueDate(\''+ htmlEscape(event.endT || '') +'\')" datetime="' + (htmlEscape(event.startT || '') || '&nbsp;') + '"></time>' +
            '<time onClick="fnCmdChangeDueDate(\''+ htmlEscape(event.endT || '') +'\')" datetime="' + (htmlEscape(event.endT || '') || '&nbsp;') + '"></time>' +
            '<div>' +
                '<em>' +(htmlEscape(event.completecnt || '') || '&nbsp;') + '</em>' +
                '<em>' +(htmlEscape(event.notcompletecnt || '') || '&nbsp;') + '</em>' +
            '</div>';

		return '<a class="classCon ' + classes.join(' ') + '"' +
				(event.url ?
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		$('<a href="#none" class="close" onClick="fnCmdCalendarClose()" title="닫기">닫기</a>').prependTo(element);
	}
