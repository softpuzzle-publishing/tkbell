	...
	$.extend($.fn, {
		...
		navSlider: function() {	
			var init = function(el) {
				...
				$(el).find('nav').on('click','a',function() {
					var slideWidth = 0;
				...
				ul.find('li a').on('click',function(){
					...
					if(!$(this).parent().hasClass('with-sort')) {
						div.width('100%');
						$(el).find('.sort').fadeOut('fast');
					} else {
						div.width( $(el).width() - sortW);
						$(el).find('.sort').fadeIn('fast');
					}
...
//	console.log($(window).width());
	...
	$('.comContents-tit').navSlider();
	$('.quiz-tags-list').navSlider();
    ...
    initMainFavNav($('.sblogPromotion > ul'));
    initMainFavNav($('.booksArea > ul'));
    initMainFavNav($('.youtubeQTab'),($(".youtubeQTab li.active").index())+1);
	...
	/* youtubeQTab에 카테고리 클릭시 pager 트리거 */
    $('.youtubeQTab a').on('click',function(){
    	var idx = $(this).closest('li').index();
    	$('.youtubeQ').find('nav.cnt a').eq(idx + 1).trigger('click');
    });
    ...
    function headerFix(){
    	...
    	if(top > 163) {
            ...
            //$('.fullpage-modal').addClass('fix');
        } else if(top < 163) {
            $('#header').removeClass('fix');
            $('aside').removeClass('fix');
            $('.floating-bnn').removeClass('fix');
            //$('.fullpage-modal').removeClass('fix');
        }
    }
    headerFix();
...
function initMainFavNav(obj,crt){
    if(!obj.get(0)) return;
	...
	if(crt == null) crt = 1;
    nav.attr('data-length',cnt).attr('data-current',crt - 1);
    ...
    nav.find('a').eq(crt-1).addClass('current');
    
    if(nav.hasClass('cnt')){
	    nav.append('<div class="cnt"><em>'+ crt +'</em><span>'+ cnt +'</span></div>');
    }
    ...
    nav.on('click','a:not(.prev):not(.next)', function () {
        ...
        if(navi.hasClass('cnt')){
            ...
            if(obj.closest('section').hasClass('youtubeQ')){
            	var tempIndex = $(this).closest("nav").attr("data-current");
		 		 fnGetYoutubeContents($(".pager li:eq("+tempIndex+") a").data('seq'));
		 	 }
        }else {
        	/* KKS #22187 MainPage 중앙의 롤링 베너 수정 요청 건 */
    ...
    if(obj.hasClass('autoRolling') && nav.find('a:not(.prev):not(.next)').length > 1){
		...
		if(obj.closest('section').hasClass('booksArea') || obj.closest('section').hasClass('youtubeQ')){
    		setTime = 10000;
    	}
    	// KKS 18524 rolling set time 6    	
    ...
    var eventRollingTimer = setInterval(function(){
    		if($(obj).find(':hover').length == 0 && $(nav).find(':hover').length == 0 && obj.find("a:focus").length == 0) {
	    		var next;
	    	/*	if(nav.children().hasClass('pager')) {
	    			next =  nav.find('a.current').closest('li').next('li').find('a');
	    			if(!next.get(0)) next = nav.find(' a:not(.prev):not(.next)').first();
	    		}else{
	    			next =  nav.find('a.current').next('a:not(.prev):not(.next)');
...
//메인 카테고리 컨텐츠 새창이동시 선택한 카테고리 유지
function fnPageBlog(url){
	localStorage.setItem("mainCategoryHistory", $(".tabs-nav ul li.active").index());
	location.href = url;
}
